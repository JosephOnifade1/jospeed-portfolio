...
const TESTIMONIALS = [
  {
    quote: "Jospeed rebuilt our booking flow in days, not weeks. Communication was async and effortless across time zones.",
    name: 'Jurre B.',
    place: 'Netherlands 🇳🇱',
    platform: 'Via Fiverr',
  },
  {
    quote: "Fast, clean Supabase work and he actually explains decisions instead of just shipping code blindly.",
    name: 'Kenneth M.',
    place: 'USA 🇺🇸',
    platform: 'Via Fiverr',
  },
  {
    quote: "The automation he built saves our team hours every week. Exactly the kind of builder you want on a lean team.",
    name: 'Raj P.',
    place: 'UAE 🇦🇪',
...
function TestimonialCard({ t, i }) {
  ...
  return (
    <div ref={ref} className={`reveal ${inView ? 'in-view' : ''}`} style={{ transitionDelay: `${i * 0.1}s` }}>
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 28, height: '100%', position: 'relative' }}>
        <Quote size={28} color="var(--indigo)" style={{ opacity: 0.3, marginBottom: 14 }} />
        <div style={{ display: 'flex', gap: 3, marginBottom: 14 }}>
          {Array.from({ length: 5 }).map((_, idx) => (
            <Star key={idx} size={14} fill="var(--cyan)" color="var(--cyan)" />
          ))}
        </div>
